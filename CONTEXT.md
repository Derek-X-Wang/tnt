...
**Appshot**:
A frontmost-window capture bundling two signals together: a window **image** (JPEG; optional — M4a Appshots are text-only) and the window's **Window Text**. Captured on demand two ways — the user presses the **Appshot Hotkey** (arms it ahead of speaking), or, mid-Voice-Turn, the Realtime model invokes a **screen tool** because the user said "look at this" / "can you see…". The screen tools are **tiered** (ADR-0006 amendment): `read_screen_text(question)` returns a capped, structured Window-Text snapshot directly as the tool output — cheap, same-turn, no second model; `analyze_screen(question)` (M4b) routes image + Window Text through the vision-capable **Cognitive Engine**, never the Realtime model. The Realtime model decides escalation from text to vision. `read_screen_text` never consumes armed Appshots; `analyze_screen` consumes them on a successful answer. Either way TNT decides the sources — armed Appshots PLUS a labeled fresh grab of the current frontmost window (#119 mixed mode), each tagged with its kind and capture age so the model answers about the right window and names its source; a same-window armed duplicate is superseded by the fresh grab for that snapshot only. Held in memory only, never written to disk by default. An **armed** Appshot persists until consumed or cleared (Capture Chip clear-all / clear-last) — it survives turns where the model never engages a screen tool, so a deliberate capture is never silently lost. While any Appshot is armed the Capture Chip continuously shows it (including between turns); in-memory screen content is never invisible. A voice-pulled fresh grab joins the same `appshots` list for the turn and clears when the turn ends. Permissions: Accessibility for Window Text (M4a needs nothing else); Screen Recording only for the image tier (M4b, JIT on first `analyze_screen`). Named after the equivalent Codex feature.
