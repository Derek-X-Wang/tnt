...
- **2026-06 — Capture split to raw AUHAL; the single-engine design is superseded (issue #136 / #141 / #142).** The plain-HAL `AVAudioEngine` capture above had a worse failure than cold-start latency: after a long idle the engine's `pause()`→`start()` resumed **output only** and the input tap **silently never fired** — `start()` returned no error, `isRunning == true`, `installTap` succeeded, yet **zero frames** were captured (a silent dead turn), and `inputNode.outputFormat(forBus:)` blocked ~5 s on the stale device. Deep research (Apple docs/forums + the production OSS dictation app VoiceInk) showed the canonical fix is to **not use AVAudioEngine for capture at all**.

  **Decision:** capture now uses a **raw CoreAudio AUHAL unit** (`kAudioUnitSubType_HALOutput`, input-only) — `CoreAudioInputUnit` → `CaptureFloatRing` (lock-free SPSC, render thread → off-thread consumer) → `NativeCapturePipeline` (conversion/chunking off the realtime thread) → `frames`. The unit is kept **initialized-but-stopped** between turns (warm device, instant start, mic indicator off). The native format is read **synchronously** via `AudioUnitGetProperty(kAudioUnitProperty_StreamFormat)` — no blocking `outputFormat(forBus:)`. A `CaptureControlCore` reducer owns lifecycle + a zero-frame watchdog (`captureStalled` → "didn't catch that"). **Playback stays on `AVAudioEngine`** (output was never the problem), so the audio path is now two independent half-duplex units — fine post-VPIO (the `-10875` risk was specific to two *VPIO* full-duplex units).

  **Consequence for ADR-0004 / #77:** because capture is independent of playback, the mic indicator now clears the instant the user stops speaking (capture stops) while the reply plays — no longer lit through the whole spoken reply.

  **Remaining:** a very-long-idle (e.g. overnight) first turn still pays the ~1.5 s HAL device wake inside `AudioOutputUnitStart` (clips the first phoneme), then self-warms to instant on the next turn. This is the hardware wake with no fast-wake API (confirmed by research); unlike the old path it **recovers** (captures + answers) rather than zombies. Mitigation candidates (warm-on-earliest-hotkey-edge; instrumentation) are a tracked follow-up — wake-prewarm was rejected (a real warm requires starting the unit, which lights the dot, and the device is not guaranteed to stay warm).
