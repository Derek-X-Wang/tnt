...
- **`read_screen_text(question)` tool** in the Realtime session config; TNT resolves the sources (armed Appshots PLUS a labeled fresh grab of the current frontmost window — #119 mixed mode; a same-window armed duplicate is superseded by the fresh grab) and returns a structured, capped Window-Text snapshot directly as the tool output — no Cognitive Engine call. Each source carries its kind (`armed_appshot` / `fresh_grab`) and capture age; the snapshot instructs the model to name the window it answers from. When Appshots are armed, TNT injects "N appshots attached" into the session.
